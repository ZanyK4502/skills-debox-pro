...
interface HomePageContentProps {
  categoryStatuses: Record<string, "ready" | "soon">;
}

export function HomePageContent({ categoryStatuses }: HomePageContentProps) {
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {categories.map((category, index) => (
            <CategoryCard
              ...
              category={category}
              index={index + 1}
              displayStatus={categoryStatuses[category.slug] ?? "soon"}
            />
          ))}
        </div>
      </section>
